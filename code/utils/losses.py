...
CLASS_NUM = [570, 580, 554, 550, 585]
# CLASS_NUM = [106, 93, 100, 112, 102, 98, 89, 103, 101, 100, 90]
# CLASS_NUM = [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000]
...
def entropy_loss(p, C=2):
    ...
    """
    y1 = -torch.sum(p * torch.log(p + 1e-6), dim=1) / np.log(C)
    return torch.mean(y1)

def entropy_minimization_loss(output):
    """
    Calculate the entropy minimization loss.
    Encourages the model to produce confident predictions.
    """
    softmax_output = F.softmax(output, dim=1)
    return entropy_loss(softmax_output)